...
awk '/n_hidden : 1000/ && /learning_rate : 0.001/ {print $0}' \
    mnist-mse-results.txt > model-1000-0.001-results.txt

awk '/n_hidden : 1000/ && /learning_rate : 0.01/ {print $0}' \
    mnist-mse-results.txt > model-1000-0.01-results.txt

awk '/n_hidden : 500/ && /learning_rate : 0.001/ {print $0}' \
    mnist-mse-results.txt > model-500-0.001-results.txt

awk '/n_hidden : 500/ && /learning_rate : 0.01/ {print $0}' \
    mnist-mse-results.txt > model-500-0.01-results.txt
