...
awk '/n_hidden : 1000/ && /learning_rate : 0.001/ {print $0}' \
    mnist-ace-results.txt > model-1000-0.001-results.txt

awk '/n_hidden : 1000/ && /learning_rate : 0.01/ {print $0}' \
    mnist-ace-results.txt > model-1000-0.01-results.txt

awk '/n_hidden : 500/ && /learning_rate : 0.001/ {print $0}' \
    mnist-ace-results.txt > model-500-0.001-results.txt

awk '/n_hidden : 500/ && /learning_rate : 0.01/ {print $0}' \
    mnist-ace-results.txt > model-500-0.01-results.txt
...
do
    ...
    awk 'BEGIN {sum=0; sq_sum=0; n=0} \
         ...
         printf("\tValidation Mean: %.3f; Std.: %.5f\n", mean*100, std)}' $fname

    awk 'BEGIN {sum=0; sq_sum=0; n=0} \
         // {sum+=$23; sq_sum+=$23^2; n++} \
         END {mean= sum/n; std=sqrt((sq_sum - (sum^2)/n)/n); 
         printf("\tTest Mean: %.3f; Std.: %.5f\n", mean*100, std)}' $fname
