...
def read_model(file_name):
    """Summarize information about a model.
    """
    pass
...
def save_predictions(model, file_name):
    """Given a model, save only its predictions to a file
    ...
    Output
    ------
    Saves only the predictions corresponding to the model in a separate file.
    """
    pass


def get_test_predictions(file_name, save_output=False):
    """Get the predictions made by a model on test data.

    Input
    -----
    file_name : string
      Path to the cPickle file containing the saved model, it's predictions and
      other training/evaluation information.

    Output
    ------
    predictions : ???
      List of predictions made by the model over folds, etc.
    """
    grid_point = cPickle.load(gzip.open(file_name, 'rb'))
    
    if save_output == True:
        pass
        # Replace "model" in the file name with "predictions" and save it to
        # the same folder as the model with the rest of the file name intact.
